...
const checkInSchema = z
  .object({
     ...
     * advance stop being required so a guest can be checked in immediately,
     * and the booking is flagged so the amount can be filled in later.
     */
    ownerWillSettle: z.boolean().default(false),
    specialRequests: z.string().optional(),
  })
  .superRefine((data, ctx) => {
    // Rent is still required for a normal check-in; only the owner-settles
  ...
  async function onSubmit(values: CheckInFormValues) {
    ...
    try {
      ...
      for (let i = 0; i < values.rooms.length; i++) {
        ...
        formData.append("roomRate", String(ownerSettles ? 0 : entry.roomRate));
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            {/*
              Lets reception complete a check-in when the owner is handling the
              money separately, instead of inventing a rent just to get past
              the form. The booking is saved with the amounts left at zero and
              flagged so it shows up as still to be settled.
            */}
            <label
              htmlFor="ownerWillSettle"
            ...
            >
              ...
              <span className="space-y-0.5">
                ...
                <span className="block text-xs text-muted-foreground">
                  Complete the check-in without entering rent or advance.
                </span>
              </span>
            </label>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                ...
                <Select value={watch("paymentMethod")} onValueChange={(value) => setValue("paymentMethod", value as CheckInFormValues["paymentMethod"])} disabled={ownerWillSettle}>
